...
class Command(BaseCommand):
    help = 'Install reviewed dimensional analysis content; preserve educator edits.'
    ...
    def handle(self, *args, **options):
        ...
        revised = json.loads((settings.BASE_DIR / 'seed' / 'measurement.json').read_text(encoding='utf8'))['01/dimensions']
        ...
        for row in revised:
            if not lesson.blocks.filter(key=row['key']).exists():
                ...
                block.save()
                added += 1
        lesson.metadata = {**(lesson.metadata or {}), 'dimensions_edition': 'pdf-1.3-v2'}
        lesson.save(update_fields=['metadata'])
        self.stdout.write(f'Dimensions: {added} added, {archived} unchanged starters archived, {kept} edited starters preserved. Publication status unchanged.')
