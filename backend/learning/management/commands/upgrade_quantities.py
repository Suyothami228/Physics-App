...
class Command(BaseCommand):
    help = 'Install reviewed physical quantities content; preserve educator edits.'
    ...
    def handle(self, *args, **options):
        ...
        revised = json.loads((settings.BASE_DIR / 'seed' / 'measurement.json').read_text(encoding='utf8'))['01/quantities']
        ...
        for row in revised:
            if not lesson.blocks.filter(key=row['key']).exists():
                ...
                block.save()
                added += 1
        lesson.metadata = {**(lesson.metadata or {}), 'quantities_edition': 'pdf-1.2-v2'}
        lesson.save(update_fields=['metadata'])
        self.stdout.write(f'Quantities: {added} added, {archived} unchanged starters archived, {kept} edited starters preserved. Publication status unchanged.')
