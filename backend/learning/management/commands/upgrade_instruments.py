...
class Command(BaseCommand):
    help = 'Install reviewed vernier caliper content; preserve educator edits.'
    ...
    def handle(self, *args, **options):
        ...
        revised = json.loads((settings.BASE_DIR / 'seed' / 'measurement.json').read_text(encoding='utf8'))['01/instruments']
        ...
        for row in revised:
            if not lesson.blocks.filter(key=row['key']).exists():
                ...
                block.save()
                added += 1
        lesson.metadata = {**(lesson.metadata or {}), 'instruments_edition': 'vernier-3d-v1'}
        lesson.save(update_fields=['metadata'])
        self.stdout.write(f'Instruments: {added} added, {archived} unchanged starters archived, {kept} edited starters preserved. Publication status unchanged.')
